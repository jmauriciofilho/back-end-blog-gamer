    ...
    companion object {
        ...
        val login = { req: Request, resp: Response ->
            ...
            if (user.password.equals(hashedPassword)) {
                ...
                logger.info("Password correto: ${token.key}")
                """"{"status":"Autenticado", "key":"${token.key}"}"""

            } else {

                logger.info("Password incorreto")
                """"{"status":"Erro", "description":"Ocorreu um erro na autenticação"}"""
